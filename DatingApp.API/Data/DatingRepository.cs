...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int currentLoggedUserId, bool likers)
        {
            var user = await _context.Users
                .Include(x => x.Likers)
                .Include(x => x.Likees)
                .FirstOrDefaultAsync(u => u.Id == currentLoggedUserId);

            if (likers)
            {
                return user.Likers.Where(u => u.LikeeId == currentLoggedUserId)
                 .Select(i => i.LikerId);
            }
            else
            {
                return user.Likees.Where(u => u.LikerId == currentLoggedUserId)
                    .Select(i => i.LikeeId);
            }
        }
